...
import { TechPipelineForm } from './tech-pipeline-form'
...
const SECTION_LABELS: Record<string, string> = {
  ...
  tech_pipeline: 'Sơ đồ công nghệ (pipeline)',
}
...
function renderForm(section: Section, onChange: (c: any) => void) {
  switch (section.type) {
    ...
    case 'tech_pipeline':
      return <TechPipelineForm content={section.content} onChange={onChange} />
  }
}
      ...
      <summary className="flex items-center justify-between px-3 py-2.5 cursor-pointer select-none border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-sm font-semibold text-slate-900 dark:text-white truncate">
            ...
            {(section.type === 'client_logos' || section.type === 'tech_pipeline') &&
